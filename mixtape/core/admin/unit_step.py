...
from mixtape.core.models.episode_step import UnitStep


class UnitStepActionStringFilter(admin.SimpleListFilter):
    title = 'action'
    ...
    def lookups(self, request, model_admin):
        ...
        for obj in model_admin.model.objects.select_related(
            'agent_step__step__episode__inference__checkpoint__training'
        ).all():
            environments.add(obj.agent_step.step.episode.inference.checkpoint.training.environment)
...
class UnitStepAdmin(admin.ModelAdmin):
    list_display = [
        'id',
        'unit',
        'agent_step',
    ...
    list_filter = [
        UnitStepActionStringFilter,
        'agent_step__step__episode__inference__checkpoint__training__environment',
    ]

    @admin.display(description='Environment')
    def environment_name(self, obj):
        return obj.agent_step.step.episode.inference.checkpoint.training.environment
    ...
    def formatted_action(self, obj):
        try:
            environment = obj.agent_step.step.episode.inference.checkpoint.training.environment
